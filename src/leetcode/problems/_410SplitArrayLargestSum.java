...
public class _410SplitArrayLargestSum {
    ...
    public int splitArrayV3(int[] nums, int m) {
    ...
    private int getMinimumLargestSplitSum(int[] prefixSum, int currIndex, int subarrayCount) {
        ...
        if (memo[currIndex][subarrayCount] != null) return memo[currIndex][subarrayCount];

        // Base Case: If there is only one subarray left, then all of the remaining numbers
        // must go in the current subarray. So return the sum of the remaining numbers.
        if (subarrayCount == 1) return memo[currIndex][subarrayCount] = prefixSum[n] - prefixSum[currIndex];
