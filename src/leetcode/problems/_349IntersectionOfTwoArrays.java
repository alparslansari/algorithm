...
public class _349IntersectionOfTwoArrays {
    ...
    public int[] intersectionV2(int[] nums1, int[] nums2) {
        Set<Integer> set_num1 = new HashSet<Integer>();
        List<Integer> sol = new ArrayList<Integer>();

        for(int i=0;i<nums1.length;i++) set_num1.add(nums1[i]);
        for(int i:nums2) {
            if(set_num1.contains(i))
            {
                sol.add(i);
                set_num1.remove(i);
            }
        ...
        int[] ints = sol.stream().mapToInt(Integer::intValue).toArray();
        return ints;
    }
}
